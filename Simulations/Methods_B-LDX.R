...
#Please follow 1000GenomesSumstatsDataPrep.R code to produce allchrs4 PLINK bfiles
...
if(CaG){
  ...
  if(simuType=="n = 10,139"){
    warning("This type of simulation cannot be executed using 1000 Genomes and a smaller n.2 and n.3 sizes. Hence, n.2 and n.3 aren't reduced")
  }
...
for (k in 1:20) {
  ...
  stopCluster(cl)
  saveRDS(out_lassosumGenCov,file = "results_lassosumExt_s_0.5.Rdata")
  ...
  multi_auto_SKZ <- snp_ldpred2_auto(corr0, df_beta_SKZ, h2_init = h_obs_SKZ, vec_p_init, ncores = NCORES, sparse = TRUE)
